...
최근 1·7·30·90일의 비교 가능한 전체 산책 A와 해당 행동 기록이 있는 산책 B를 같은 지도에서
...
처음에는 최근 30일을 조회한다. 조회량 상한으로 실패하면 7일 또는 1일로 줄여 다시 조회할 수
있다. 기간은 서울 날짜 기준 양끝을 포함하므로 1일은 오늘 하루다. 기간 버튼은 가로로 스크롤하며
새로고침 버튼은 고정한다. 하루 범위도 서버 상한을 넘으면 오류를 유지하며 일부 결과로 바꾸지 않는다.
...
2026-09-10 기간 축소 수정에서는 `WalkBehaviorComparisonScreenTest`와
`WalkBehaviorComparisonApiTest`만 실행해 **8개 통과, 실패·skip 0개**를 확인했다.
320dp 화면에서 30일·90일의 413 오류 뒤 7일·1일 선택이 실제 요청 날짜와 성공 결과에
반영되는 흐름을 포함한다. 앱 세션·Room 접근을 얇은 화면 진입부에 남겨 두고, 같은
상태·조회 흐름을 쓰는 Browser에 테스트 조회 함수를 주입했다. 지도 슬롯은 대체했다.

```powershell
.\gradlew.bat :app:testDebugUnitTest --tests 'com.daengs.app.ui.walk.WalkBehaviorComparisonScreenTest' --tests 'com.daengs.app.walk.diary.WalkBehaviorComparisonApiTest'
```
