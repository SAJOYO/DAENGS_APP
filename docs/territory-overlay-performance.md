...
경로 선을 구간별로 재사용하고 변하지 않은 구간의 속도색 가공 결과를 캐시했다. 실제 폰 4구간/1만 좌표에 20번 추가하는 조건에서 생성·제거 각각 80→0회, 가공 좌표 200,210→50,210개였다. 현재 늘어나는 구간의 전체 가공은 남는다. [구현 경계·대상 테스트·실기기 측정](walk-route-performance.md)에 재현 방법과 한계를 기록했다.
