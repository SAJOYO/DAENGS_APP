...
PR #409부터 입력·대기·답변은 최대 324dp 폭과 기본 246dp 높이의 같은 대화판을 쓴다.
답변 후에도 하단 입력칸을 유지해 되묻기에 바로 답한다. 입력 중에도 최신 답변을 유지하며
전송한 문장은 비운다. 보내지 않은 초안은 닫았다 열어도 유지하고 키보드는 자동으로 열지 않는다.
큰 글씨에는 여유 높이를 더하고, 긴 답변은 대사 영역 안에서 스크롤한다. 이름표와
발바닥 장식은 기본 글꼴 및 기존 테마를 사용한다.

도우미견 입력의 키보드는 결과 카드 위를 덮는다. 카드의 접힘/미리 보기/펼침 상태와
목록 스크롤, 지도 크기는 그대로 유지한다. 키보드를 피하려고 카드를 자동으로 접지 않는다.
대화판은 버튼 위에 붙되 키보드에 가리지 않도록 표시 가능한 창 범위 안에서 이동한다.
장소명 등 다른 입력창에는 기존 키보드 회피 배치를 적용한다.

키보드가 보일 때 뒤로가기는 키보드만 내리고 대화와 입력 문장을 유지한다. 키보드가
없는 상태의 뒤로가기와 닫기 버튼은 대화창을 닫는다. 전송으로 입력칸이 먼저 사라져도
키보드가 내려가는 동안 지도와 결과 카드가 다시 줄어들지 않도록 입력 소유 상태를 유지한다.
