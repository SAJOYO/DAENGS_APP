...
- 실제 화면 좌표로 투영한 연속 경로에서 앞뒤 접선을 구하고 곧은 방향 후보를 먼저 검사한다.
- 화면 끝·장면/출발/종료 마커·지도 설정 버튼·다른 경로·다른 화살표를 피한다.
- 원본 간선·비율로 이전 후보를 우선 재검사한다. 진행 방향 오른쪽 16dp, 최대 5개로 제한한다.
  충돌하면 생략하며 반대편·원거리로 옮기거나 개수를 강제로 채우지 않는다.
...
PR #419의 정책 소유권·수치와 검증 방법은 [외부 방향 표시](walk-direction-policy.md)에 있다.
