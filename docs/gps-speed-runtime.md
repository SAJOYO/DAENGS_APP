...
일시정지·종료 버튼과 기존 상태 모달은 그대로다. 새 엔진의 거리·시간 snapshot을 경로, 핀, 산책 요약, 업로드에 사용하지 않는다. #313에서 [산책별 정책 저장/로드와 완료 기록 비교](gps-policy-persistence.md)를 연결했다. 시작 때 만든 동일 정책을 런타임과 세션 저장에 사용한다. 새 거리 판정 활성화는 후속 5-2 전환 작업이다.
