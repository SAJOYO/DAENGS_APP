...
- WorkManager의 네트워크 조건과 지수 backoff로 전달한다. 같은 세션의 작업은 저장 순서를
...
공유 조회의 `site.occupancy`와 버전이 현재 점유의 근거다. 최초 응답에 포함된 사이트 버전이
조회 캐시보다 새로우면 즉시 반영하고 이후 조회로 갱신한다. 과거 `GRANTED`는 현재 소유권을 보장하지 않는다.
저장된 과거 응답을 현재 점유로 덮어쓰지 않는다.
