...
위 측정은 #329의 구간 전체 가공 구현을 기준으로 한다. 후속 [단일 경로 끝부분 캐시](walk-route-tail-cache.md)는 append 시 직전 선분과 새 부분만 다시 보간한다. SDK 전체 좌표 갱신, 상위 TrailSnapshot→지도 상태의 전체 좌표 투영과 복사 비용은 남는다. 앞쪽 좌표가 잘리는 경우도 전체 가공한다.
