...
검색창과 기간·계절·출발 날씨 조건은 기록 유무와 무관하게 표시한다.
현재 제목·사용자 메모 검색과 페이지 전 조건 적용은 [산책 기록 검색](walk-history-search.md)을 따른다.
...
관측 완전성 분모와 계산 대상 식별자를 가진 누적 브러시 계약은 후속 작업이다. 단순 목록 페이지를 누적 집계의
