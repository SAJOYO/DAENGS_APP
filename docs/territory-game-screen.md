...
지도 버튼은 산책 지도에 TERRITORY 표시를 요청한다. 여기서 들어간 지도의 뒤로 가기는 게임 화면으로 돌아오고, 지도 홈 버튼은 홈으로 간다. 조회 강아지 선택과 성적 스크롤 위치는 화면을 다녀와도 복원한다. 화면 이동은 산책 시작/정지/참여견 변경 명령을 보내지 않는다. 산책 중이면 버튼에 산책 지도로 돌아가기를 표시한다.
