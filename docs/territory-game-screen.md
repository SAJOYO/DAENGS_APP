...
- 점령하기는 실제 전봇대와 범위 색을 재사용하는 로컬 예시다. 주인과 점수는 가상의 두부/초코이며, 예시라는 문구를 항상 표시한다. 로그인·게임 활성화 없이도 읽을 수 있다.
...
- 촬영 구도는 기존 비숑 전신 스프라이트와 전봇대를 한 프레임에 넣는다. 실제 카메라·위치·산책·점령 API를 실행하지 않는다. 실제 성공은 서버 인증 후 확정된다는 설명을 포함한다.
...
![강아지와 전봇대를 함께 촬영하는 구도](territory-game/game-rules-photo.png)
