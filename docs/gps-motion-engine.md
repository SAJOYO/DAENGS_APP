...
상태: #294에서 순수 엔진·완료 저널 재생을 구현했고, #307에서 [운영 서비스의 속도 표시에 연결했다](gps-speed-runtime.md). #313에서 [세션별 정책 저장과 완료 기록 비교](gps-policy-persistence.md)를 연결했다. 사용자 거리·요약의 전환은 아직 적용하지 않았다.
...
같은 설정의 저장·복원과 동일 입력의 step 결과까지 테스트했다. #313의 비교 조회는 소유자를 검사하고 저장된 정책을 사용한다. 과거 기록 전체를 새 규칙으로 다시 쓰거나 서버 정책과 동일하다고 주장하지 않는다. 새 거리·경로의 활성화는 후속 5차 전환에서 수행해야 한다.
