...
2026-09-12, 구현 `f64d1ce3`에 최신 dev `503fdc9b`를 통합한 `5c124396`에서 위 명령을 실행했다.
JDK 25·Android SDK 37의 Debug 빌드와 **7개 클래스 86개 테스트가 통과**했다. 실패·오류·skip은 0개다.
수신 확인 11개, HTTP 7개, Room 연계 1개, 기존 동기화 24개, GPS 증거 8개, 측정 동기화 16개,
Room DAO 19개다. 전체 테스트 실행 결과는 아니며 서비스 설정 없는 로컬 Debug 빌드다.
