...
새 응답의 행동 문장에는 현재 핀의 원본 ID가 별도 제공되지 않는다. `originals`는 주로 메모/사진이다.
장면 ID나 같은 시각/좌표로 핀 ID를 추측하지 않는다. 원본 행동 편집은 기존 원본 기록 경로에 남고,
일기 장면 편집/숨김은 장면 자체에 적용한다. 핀별 아이콘/직접 편집 연결에는 서버의 명시적 참조가 필요하다.
