...
object RoomSpec {
     ...
     * 놀았다. 그 여백을 방에 준다 (`f06d1419`, 2026-08-28).
     *
     * ## ⚠️ 이 값을 올려서 방을 키우려 하지 말 것 (2026-09-18)
     *
     * 사용자가 "방이 기종마다 너무 다르게 나온다" 고 해서 **"각 화면에 맞춰 최대한
     * 크게"** 로 방향을 정했고, 그 답으로 이 값을 1.46 까지 올려 봤다. 실기기에서 보고
     * **거절했다** — *"방이 좌우로 그냥 늘려진 느낌"* · *"좌우로 잡아 당겨서 하라는 게
     * 아니야"*.
     *
     * 맞는 지적이다. 이 값은 **균일 배율이 아니라 가로만 당기는 것**이라, 키우는 대신
     * 아이소메트릭 원근을 납작하게 만든다. 「최대한 크게」의 답이 아니다.
     *
     * | 값 | 방 그림 폭 | 좌우 여백 | |
     * | --- | --- | --- | --- |
     * | 1.00 | 260.6dp | 75.4dp | 비율 정확. 대신 방이 지금보다 좁다 |
     * | **1.18** | **306.9dp** | **52.0dp** | 지금 값 |
     * | 1.46 | 380.0dp | 15.4dp | 시안 목표(20dp)에 닿지만 **늘려 보인다** |
     *
     * (Pixel 3 XL · 방 상자 347.6dp 실측)
     *
     * **1.0 도 후보였다.** 왜곡을 아예 없애는 값이다. 그런데 1.18 은 `f06d1419` 가
     * **"방을 가로로 늘리고"** 라는 목적으로 넣은 값이라, 1.0 으로 가는 것은 되돌리기가
     * 아니라 **그 결정을 뒤집는 일**이다. 사용자가 보고 **1.18 을 그대로 두기로 했다.**
     *
     * ## 그러면 비율을 지키며 방을 키우는 길은
     *
     * **상자를 키우는 것뿐이고, 그건 같은 날 따로 했다** — 하단바·시즌 행·챗봇 카드에서
     * 72dp 를 되찾아 방 상자가 275.6 → 347.6dp (+26%) 가 됐다. [INSET] 주석이
     * *"좌우 여백을 줄이고 싶으면 이 상수가 아니라 방 상자 자체를 키워야 한다"* 고
     * 가리킨 바로 그 길이다.
     *
     * 남은 여유도 재봤다. 칠해진 영역([ART_PAD_TOP] 등)으로 맞춰도 `1402/1363 = +2.9%`
     * 뿐이고, [INSET] 을 1 로 올리는 것은 *"위아래가 상자에 딱 붙어 잘린 것처럼 보인다"*
     * 는 이유로 이미 거절된 값이다. **비율을 지키는 한 여기가 최대치다.**
     * 더 키우려면 **가로형 방 아트**가 와야 한다 (STATUS 「다음 결정」 5번).
     *
     * ## 늘리면 무엇이 같이 늘어나나
     *
     ...
     * 에 실제로 그럴 뻔했다.
     *
     * 값을 다시 보려면 **개발자 패널의 「가로」 슬라이더**로 실기기에서 본다 (디버그 전용).
