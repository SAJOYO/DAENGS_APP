...
object InviteShare {
     ...
     * 읽힐 여지를 준다.
     *
     * **누르는 길을 먼저, 붙여넣는 길을 대안으로 적는다.** 운영 서버에 App Links 서명
     * 지문이 들어가(2026-09-17, SAJOYO/DAENGS_dev#581) 설치된 앱이면 링크를 누를 때 초대가
     * 바로 열린다. 다만 **"누르면 반드시 앱이 열린다" 고 쓰지 않는다** — 카카오톡 안의
     * 브라우저는 앱으로 넘기지 않고 웹 안내(`/invite`)를 먼저 띄우고, 앱이 옛 버전이면 링크를
     * 모른다. 그때를 위해 「받은 초대 링크 넣기」 붙여넣기를 3번에 그대로 남긴다.
     */
    fun message(petName: String?, inviteLink: String): String {
        ...
        return buildString {
            ...
            append("2. 아래 링크를 누르면 앱에서 초대를 열 수 있어요.\n")
            append("3. 앱이 열리지 않으면 이 메시지 전체나 링크를 복사해 앱의 「받은 초대 링크 넣기」에 붙여넣어 주세요.\n")
            append("\n")
