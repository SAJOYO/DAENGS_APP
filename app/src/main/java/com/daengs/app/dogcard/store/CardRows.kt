...
data class DrawnCardRow(
     ...
     *
     * 예전에는 얼굴을 앱이 알아서 구멍에 끼웠다 — 또렷한 얼굴 사각형을 1.15배로
     * 키우고([CORE_OVERFILL]) 턱을 구멍 아래에 걸었다. 둘 다 **사용자가 못 보고
     * 맡겼을 때** 필요한 보정이다.
     *
     * 이제는 원 안에서 직접 확대·이동해 맞춘다. 그렇게 맞춘 카드에 그 보정을 또 걸면
     * **본 것과 다르게 나온다.** 그래서 갈라야 하고, 갈리는 기준이 이 칸이다.
     *
     * `0` 인 옛 카드는 예전 규칙 그대로 그린다. **이미 뽑아 둔 카드가 업데이트로
     * 달라지면 안 된다.**
     */
