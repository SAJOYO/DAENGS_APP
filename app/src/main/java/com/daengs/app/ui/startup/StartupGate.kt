 ...
 * 둘을 같은 값으로 다루면 강아지를 못 받은 사이에 방이 잘못 그려진다.
 *
 * **`Onboarding` 이 없어졌다.** 예전에는 강아지가 없으면 여기서 등록 화면으로 보냈다 —
 * 로그인하자마자 남의 정보를 채우게 만드는 자리였고, 게다가 빠져나갈 수도 없었다
 * (`MainActivity` 가 `onCancel` 을 없앴다). 이제 강아지 없이 방까지 들어가고,
 * 강아지가 있어야 하는 기능을 누를 때 그 자리에서 청한다 ([com.daengs.app.ui.home.PetNeed]).
 */
 ...
 * 안 왔고([Pet] 목록이 `null`), 방은 목록이 비면 **데모로 채워졌다** — 그래서 켤 때마다
 * 남의 강아지 넉 마리가 잠깐 서 있다가 사라지고 날씨가 바뀌었다. 지금은 로그인한
 * 사람의 빈 목록이 빈 방이 되지만([com.daengs.app.ui.home.roomRoster]), **여기서
 * 기다리는 이유는 그대로다** — 못 받아 온 사이에 빈 방을 보여 주면 그것도 거짓말이다.
 *
