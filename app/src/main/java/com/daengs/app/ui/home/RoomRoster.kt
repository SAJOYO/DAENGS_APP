 ...
 * @param waitsForPet 로그인했는데 **아직 아이가 없나** ([needsPet]). 빈 목록의 뜻이
 *   이것으로 갈린다 — 아래 ②·③. 개발자 패널의 "빈 방으로 보기" 도 이 값을 켠다
 */
fun roomRoster(pets: List<Pet>?, waitsForPet: Boolean): List<DogBreed> = when {
    ...
    // ② 로그인했는데 한 마리도 없다 → **빈 방이다.**
    //
    // 예전에는 이것도 아래 ③ 으로 묶어 데모를 세웠다. 근거는 "온보딩으로 넘어가기
    // 직전의 짧은 사이" 였는데, **강아지 등록이 더 이상 강제가 아니라서 그 전제가
    // 없어졌다** — 이제는 짧은 사이가 아니라 오래 머무는 정상 상태이고, 그동안 세워 둔
    // 데모는 견본이 아니라 **내 아이로 오해된다.** 바로 위 ① 이 같은 이유로 이미 비운다.
    //
    // ⚠️ **빈 방은 「강아지 데려오기」 자리와 함께여야 한다** ([EmptyRoomInvite]).
    // 그것 없이 비우면 아래 [roomPets] 주석대로 "고장 난 것" 으로 읽힌다.
    //
    // **③ 보다 먼저 본다.** 개발자 패널의 "빈 방으로 보기" 가 이 갈래를 강제로 켜는데,
    // 뒤에 두면 둘러보기(빈 목록)가 ③ 에 먼저 걸려서 토글이 아무 일도 안 한다.
    waitsForPet -> emptyList()

    // ③ 로그인 전인데 한 마리도 없다 → 견본을 세운다.
    //
    // **랜딩의 "둘러보기"** 다 (`app/src/debug/.../SkipBrowse.kt` — 디버그 전용).
    // 거기는 계정이 없어 "내 강아지" 라는 개념 자체가 없으므로, 데모를 남의 아이로
    // 오해할 일도 없다. 출시본은 로그인이 필수라 이 갈래에 닿지 않는다.
    //
    // ⚠️ **방 둘러보기(튜토리얼)와 다른 말이다** — 그건 `RoomTour.kt` 이고 전원이 본다.
 ...
 * 번호가 뒤의 것의 첨자라, 거르는 자리를 둘로 나누면 반드시 어긋난다 — **배웅한 아이의
 * 하트가 남의 아이 곁에 뜬다** (`MiniRoomDrawing` 의 `drawHeart`). 마이 목록의 무지개
 * 아이콘과는 다른 표시다.
 *
