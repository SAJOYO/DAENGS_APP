...
// 포일 대부분이 `color-dodge` 라 밝은 원화에 얹으면 흰색으로 클리핑되고, **제일 먼저
// 사라지는 게 흰 강아지 얼굴**이다. 실기기에서 보면 비글이 누렇게 뜨고, 가지 카드에서는
// 눈이 한쪽은 보라 한쪽은 초록이 된다.
//
// 그때는 포일 세기를 통째로 0.30 까지 낮춰 막았는데, 그러면 반짝여야 할 금속 프레임까지
// 같이 죽는다. **세기가 아니라 자리를 가른다.** (2026-09-18 에 그 0.30 을 걷고 웹판의
// 포일별 세기 [webTune] 으로 되돌렸다 — 얼굴은 이 파일이 지킨다.)
//
...
// 그런데 `foilLayer` 호출 지점이 수십 곳이다 (포일마다 shine · `::before` · `::after` · glare).
//
