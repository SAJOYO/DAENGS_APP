...
internal class DiaryReadingMemory(val drawer: DiaryDrawerState, val list: LazyListState) {
    ...
    suspend fun restore(value: JSONObject) {
        // Earlier layouts put presets and explanations elsewhere; restore the route reading at its start.
        pendingExplorerOffset = if (value.optInt("explorerLayout") == 4) value.optInt("explorerOffset", 0).coerceIn(0, 100_000) else 0
