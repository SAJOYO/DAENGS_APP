 ...
 * 이머시브를 **버전 1 에 안 넣는다.**
 *
 * 무대는 그림 넉 장(배경·주인공 누끼·진입 카드·창틀)으로 이뤄지는데, 카드가 "우리
 * 아이의 카드" 가 된 지금 **무대 안에 서 있는 주인공만 저쪽 강아지로 남는다.**
 * 진입 연출의 카드는 우리 것으로 갈 수 있어도, 무대 주인공을 바꾸려면 야채 몸통에
 * 얼굴이 합쳐진 **전신 그림**이 있어야 한다 — 우리에게는 얼굴 누끼뿐이다.
 *
 * 반쪽으로 내보내면 "내 카드로 들어갔는데 남의 개가 서 있는" 화면이 된다. 그래서
 * 전신 그림이 준비될 때까지 꺼 둔다 (2026-09-02 결정).
 *
 * **코드는 안 지운다.** 장면 세 벌과 진입 연출은 실기기에서 다듬어 둔 것이라 다시
 * 만들 이유가 없다. 이 값을 `true` 로 되돌리면 그대로 살아난다.
 *
