 ...
 *
 * ## [ScreeningApi] 와 무엇이 다른가
 *
 * 그쪽은 판정만 받아 오고 **아무것도 안 남는다.** 여기는 사진과 판정이 기록 한 줄로
 * 묶여 서버에 남는다 — "지난번보다 나아졌나" 를 보려면 그게 있어야 한다.
 *
 * ⚠️ **옛 경로를 대체하지 않는다.** 저쪽이 `/screen/v1/screen` 을 그대로 살려 뒀고,
 *    이쪽은 **토큰이 있어야** 쓴다. 로그인 안 한 상태나 저장소가 아직 안 켜진 동안
 *    (503)에는 옛 경로로 물러선다 — 진단 자체는 계속 돼야 한다.
 *
 * ## 흐름이 왜 세 걸음인가
 *
 * `기록 열기 → 사진 PUT → confirm`. 보행·프로필 사진과 같은 모양이고, 저쪽이 GCS 로
 * 되돌아가도 앱이 안 바뀌게 하려는 계약이다. **`confirm` 이 판정까지 한다** — 그래서
 * 그 응답이 0.6~3초 걸리는데, 옛 경로도 같은 시간을 기다렸으니 새로 생기는 지연이 아니다.
 *
