 ...
 *
 * ⚠️ **여기는 옛 경로다.** 기록이 필요하면 [ScreeningRecordApi] 를 쓴다 —
 *    이쪽은 판정만 받아 오고 **아무것도 안 남는다.** 그래도 지우지 않는다:
 *    로그인 안 한 사람도 진단은 쓸 수 있고, 서버 저장소가 아직 안 켜진 동안에는
 *    새 계약이 503 이다. 어느 길로 갈지는 [ScreeningRun] 이 정한다.
 *
 * ⚠️ **위 "저쪽 데모 서버" 는 옛말이다.** 저쪽이 D-040 으로 스크리닝을 backend 안
 *    (`backend/src/daengs_screening/`)으로 옮겼고, nginx 가 `/screen/` 을
 *    `backend:8000` 으로 넘긴다. 지금 이 주소는 우리 서버 PC 다.
 *    다만 **인증이 없는 것은 그대로**라, 새 계약이 생긴 이유가 그것이기도 하다.
 *
 * 주소가 비어 있으면 [configured] 가 false 이고, 화면이 그걸 보고 버튼을 막는다 —
 * 카카오 로그인과 같은 방식이다.
 */
object ScreeningApi {
