...
data class AppMe(
     ...
     * OCR 학습 이용에 동의했나 (#258).
     *
     * **원본은 시각이다** — 저쪽 `app_users.ocr_consent_at` 이고, "언제 어느 판에"
     * 가 남아야 근거가 선다. 앱에는 그 시각이 아니라 불리언으로 온다. 판 번호
     * (`ocr_consent_version`)는 서버가 정하므로 앱이 읽지도 보내지도 않는다.
     *
     * ⚠️ **이 값은 진료비 기록을 안 가른다.** 미동의여도 금액·병원·사유가 저장되고
     * 비서도 읽는다. 갈리는 것은 OCR 이 읽은 진료 항목(`raw_ocr_items`)을 학습용으로
     * 남길지 하나뿐이고, 화면에는 두 경우 모두 항목이 보인다.
     */
    val ocrConsent: Boolean = false,
)
