...
class ServerTerritoryGameProvider(
    ...
    private var ignoredReceipt: String? = actions?.receipt?.value?.claim?.claimId
    ...
    override suspend fun refresh(sites: List<TerritorySite>) {
        if (!acceptingReceipts) ignoredReceipt = actions?.receipt?.value?.claim?.claimId
    ...
    private fun actionSnapshot(base: TerritoryGameState, tracking: WalkTrackingState, permitted: Boolean,
        ...
        val sessionReady = rows.any { it.kind == "REGISTER" && it.state == "CONFIRMED" } &&
            rows.none { it.kind != "MARK" && it.state != "CONFIRMED" } &&
            rows.filter { it.kind != "MARK" }.lastOrNull()?.let {
        ...
        val confirmed = live?.claim?.takeIf { acceptingReceipts && it.claimId != ignoredReceipt && it.disposition == ClaimDisposition.GRANTED &&
        ...
        val guidance = when {
            ...
            rows.any { it.kind != "MARK" && it.failure == "auth" } -> "로그인 상태를 다시 확인해 주세요 · 요청은 보관 중이에요"
            rows.any { it.kind != "MARK" && it.state == "REJECTED" } -> "게임 세션을 연결할 수 없어요 · 산책과 참여견을 확인해 주세요"
        ...
        return base.copy(readOnly = false, sites = sites, claimingPetId = pet,
            ...
            guidance = guidance, confirmedMarkId = confirmed?.claimId, confirmedMarkSiteId = confirmed?.site?.siteId)
    }
