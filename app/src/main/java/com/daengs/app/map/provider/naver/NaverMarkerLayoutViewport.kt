...
internal fun markerExclusions(size: IntSize, density: Float, query: MapVisibilityQuery?): List<MarkerRect> = buildList {
    if (query != null) {
        ...
        if (query.topRightCoverPx > 0) add(MarkerRect((size.width-query.topRightCoverPx)/density.toDouble(),query.topRightCoverTopPx/density.toDouble(),
            size.width/density.toDouble(),(query.topRightCoverTopPx+query.topRightCoverPx)/density.toDouble()))
    }
}
