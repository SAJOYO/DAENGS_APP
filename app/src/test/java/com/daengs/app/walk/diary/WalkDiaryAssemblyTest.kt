...
class WalkDiaryAssemblyTest {
    ...
    private val empty = DiaryBoardInput(emptyList(), null, null, emptyList(), null)
    ...
    private fun assemble(input: DiaryBoardInput, photos: List<WalkPhoto> = emptyList(), draft: String? = null) =
    ...
    @Test fun `publication keeps saved prose and explicit edits while reconciling live entries and photos`() {
        ...
        val base = LocalDiaryBoard.build(walk, emptyList(), listOf(note, changed, removed), listOf(imageRow))
    ...
    @Test fun `title projection does not parse entries or reconcile publication base`() {
        ...
        val input = empty.copy(entries = rows, analysis = analysis(storyboardEntryStamp(rows)))
        assertEquals("함께 남긴 산책 기록", diaryTitle("s", input))
        ...
        assertEquals("함께 남긴 산책 기록", diaryTitle("s", published))
        assertNull(diaryTitle("other", published))
    }
    ...
    @Test fun `failed refresh retains the prior matching bundle and notice`() {
        ...
        assertEquals(diary.title, diaryTitle("s", failed))
    ...
    @Test fun `malformed published bundle still fails instead of silently replacing saved prose`() {
        ...
        assertTrue(runCatching { diaryTitle("s", input) }.isFailure)
