...
class RoomStretchTest {
    ...
    /**
     * ⛔ **1.18 은 `f06d1419`(2026-08-28)가 정한 값이고 그대로 둔다** —
     * `docs/design-locks.md` 6절.
     *
     * 2026-09-18 에 "각 화면에 맞춰 최대한 크게" 를 이 값으로 풀려고 1.46 까지 올려
     * 봤다가 사용자가 실기기에서 **거절했다** — *"좌우로 잡아 당겨서 하라는 게 아니야"*.
     * 이 값은 균일 배율이 아니라 가로만 당기는 것이라 방을 키우는 답이 아니다.
     * **비율을 지키며 키우는 길은 방 상자를 키우는 것이고, 그건 따로 했다** (+26%).
     */
    @Test
    fun `기본값은 1_18 이다`() {
